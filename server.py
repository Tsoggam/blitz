...
def get_player(username):
    ...
    c.execute(
        'SELECT chips, last_claim FROM players WHERE username = ?',
        (username,)
    )
    result = c.fetchone()
    ...
    if result:
        ...
        if time_passed >= timedelta(days=1):
            chips = DAILY_CHIPS
            c.execute(
                'UPDATE players SET chips = ?, last_claim = ? WHERE username = ?',
                (chips, now.isoformat(), username)
            )
            conn.commit()
...
def update_player(username):
    ...
    c.execute(
        'UPDATE players SET chips = ? WHERE username = ?',
        (new_chips, username)
    )
...
def get_ranking():
    ...
    c.execute(
        'SELECT username, chips FROM players ORDER BY chips DESC LIMIT 10'
    )
    results = c.fetchall()
    conn.close()

    return jsonify([
        {'name': row[0], 'chips': row[1]} for row in results
    ])
...
def get_history():
    ...
    c.execute(
        'SELECT result, winners, timestamp FROM game_history ORDER BY id DESC LIMIT 10'
    )
    results = c.fetchall()
    conn.close()
...
def add_history():
    ...
    c.execute(
        'INSERT INTO game_history (result, winners) VALUES (?, ?)',
...
def cleanup_old_data():
    ...
    c.execute(
        'DELETE FROM players WHERE last_claim < ?',
        (cutoff_date,)
    )
    deleted = c.rowcount
